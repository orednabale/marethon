...
import RegistrationItem from '../pages/RegistrationItem';
...
class RegistrationTable extends Component {
  ...
  componentDidMount() {
    ...
    const _gameInfoKey = contract.methods["getGameInfo"].cacheCall(this.props.regRound);
    const _firstPageKey = contract.methods["get1stLastPage"].cacheCall(this.props.regRound,ENT_PERPAGE,0);
  ...
  pageClick(_whichPage, fromRunner=null) {
    ...
    _whichPage == FIRST ?
    	dataPageKey = contract.methods["get1stLastPage"].cacheCall(this.props.regRound, ENT_PERPAGE, 0) :
    _whichPage == LAST ? 
    	dataPageKey = contract.methods["get1stLastPage"].cacheCall(this.props.regRound, ENT_PERPAGE, 1) :       	
    _whichPage == PREV ? 
    	dataPageKey = contract.methods["getPrevNextPage"].cacheCall(this.props.regRound, parseInt(fromRunner.slice(-13).slice(0,9)), ENT_PERPAGE, 0) :       	 
    	dataPageKey = contract.methods["getPrevNextPage"].cacheCall(this.props.regRound, parseInt(fromRunner.slice(-13).slice(0,9)), ENT_PERPAGE, 1)      	
  ...
  render() {
	    ...
	    if (gameInfo && currentPage) {
	        ...
	        <Row>
	          <Col md="9" sm="6" xs="12">
	            <Card>
	              <CardHeader><h5>Registration Board</h5></CardHeader>
	              <CardBody>
										...
										<tbody>
	            			{
					  					actualPage.length != 0 ? (
												...
												actualPage.map((runner, index) => (
													<RegistrationItem key={runner.slice(-13).slice(0,9)} runner={runner.slice(-13).slice(0,9)} { ...this.props } />
												)) 						
											)	: this.state.whichPage == PREV ? this.pageClick(FIRST)
												: this.state.whichPage == NEXT ? this.pageClick(LAST)
												: <tr>Nothing to display. Nobody registered yet</tr>
	            			}
	            			</tbody>
									</Table>
	          ...
	          <Col md="3" sm="6" xs="12">
							<Card>
								<div style={{backgroundColor:'#ededed', textAlign:'center'}}>Registration Round Info</div>
								<Table responsive size="sm" >
									<tbody>
										<tr>
											<td style={{textAlign:'right'}}>Round : </td>
											<td><b>{this.props.regRound}</b></td>
										</tr>
										<tr>
										...
										<tr>
											<td style={{textAlign:'right'}}>Registered Runners : </td>
			...
			} else return	<Card><CardTitle>Retrieving Registration Table...</CardTitle></Card>;
		}	
	}
